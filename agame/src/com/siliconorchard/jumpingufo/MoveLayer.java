...
public class MoveLayer extends CCLayer {
	...
	public final float FIRST_BODY_Y =   (40 * Common.kYForIPhone);
	public final float SECOND_BODY_Y =   (27 * Common.kYForIPhone);
	...
	{
	    lands = new ArrayList<CCSprite>((int) Common.STAGE_COUNT);
	    
	    for (int i = 0; i < (int) Common.STAGE_COUNT; i ++) {
	        
	        CCSprite land = CCSprite.sprite(Common.IMG_LAND[i]);
	        land.setScaleX(Common.kXForIPhone);
	        land.setScaleY(Common.kYForIPhone);
	        land.setPosition(Common.SCREEN_WIDTH / 2, land.getContentSize().height / 2);
	        addChild(land, Common.ztag_In_camera.ztag_land);
	...
	private void initbodys()
	{
	    ...
	    rivers1 = new ArrayList<CCSprite>(5);
	    rivers2 = new ArrayList<CCSprite>(5);
	    rivers3 = new ArrayList<CCSprite>(5);
	    rivers4 = new ArrayList<CCSprite>(5);
	    
	    for (int i = 0; i < Common.STAGE_COUNT; i ++) {
	        switch (i) {
	            case 0:
	            {
	                float startPosXFir = 0;
	                ...
	                for (int index = 0; index<4; index ++) {
	                    ...
	                    bodySec.setPosition(startPosXSec, SECOND_BODY_Y);
	                    addChild(bodySec, Common.ztag_In_camera.ztag_body2);
	                    startPosXSec += Common.SCREEN_WIDTH;
	                ...
	                for (int i1 = 0; i1 < 5; i1 ++) {
	                    ...
	                    river.setPosition(posX_river, river.getContentSize().height / 2 * Common.kYForIPhone);
	                    addChild(river, Common.ztag_In_camera.ztag_river);
	                    rivers1.add(river);
	                    
	                    posX_river += river.getContentSize().width / 2 * Common.kXForIPhone;
	                }
	            ...
	            case 1:
	            {
	                ...
	                float startPosXSec = Common.SCREEN_WIDTH / 2;
	                for (int index = 0; index<4; index ++) {
	                    CCSprite bodySec = CCSprite.sprite(Common.IMG_BG2_BACK2[index]);
	                    bodySec.setScaleX(Common.kXForIPhone);
	                    bodySec.setScaleY(Common.kYForIPhone);
	                    bodySec.setAnchorPoint(0, 0);
	                    bodySec.setPosition(startPosXSec, SECOND_BODY_Y);
	            ...
	            case 2:
	            {
	                ...
	                float startPosXSec = Common.SCREEN_WIDTH / 2;
	                for (int index = 0; index<3; index ++) {
	                    CCSprite bodySec = CCSprite.sprite(Common.IMG_BG3_BACK2[index]);
	                    bodySec.setScaleX(Common.kXForIPhone);
	                    bodySec.setScaleY(Common.kYForIPhone);
	                    bodySec.setAnchorPoint(0, 0);
	                    bodySec.setPosition(startPosXSec, SECOND_BODY_Y);
	            ...
	            case 3:
	            {
	                ...
	                float startPosXSec = Common.SCREEN_WIDTH / 2;
	                for (int index = 0; index<4; index ++) {
	                    CCSprite bodySec = CCSprite.sprite(Common.IMG_BG4_BACK2[index]);
	                    bodySec.setScaleX(Common.kXForIPhone);
	                    bodySec.setScaleY(Common.kYForIPhone);
	                    bodySec.setAnchorPoint(0, 0);
	                    bodySec.setPosition(startPosXSec, SECOND_BODY_Y);
	...
	{
	    for (int i = 0; i < Common.STAGE_COUNT; i ++) {
	        CCSprite land = lands.get(i);
	        land.setVisible(false);
	        
	    }
	    CCSprite land = lands.get(Common.g_stageNum);
	    ...
	    arrayCount = rivers1.size();
	    for (int num = 0; num < arrayCount; num ++) {
	        CCSprite body = rivers1.get(num);
	        body.setVisible(false);
	    }
	    
	    arrayCount = rivers2.size();
	    for (int num = 0; num < arrayCount; num ++) {
	        CCSprite body = rivers2.get(num);
	        body.setVisible(false);
	    }
	    
	    arrayCount = rivers3.size();
	    for (int num = 0; num < arrayCount; num ++) {
	        CCSprite body = rivers3.get(num);
	        body.setVisible(false);
	    }
	    
	    arrayCount = rivers4.size();
	    for (int num = 0; num < arrayCount; num ++) {
	        CCSprite body = rivers4.get(num);
	        body.setVisible(false);
	    }
	...
	{
	    CCSequence moveSeq = CCSequence.actions(CCMoveBy.action(0.7f, CGPoint.ccp(0, -10 * Common.kYForIPhone)),
	                  CCMoveBy.action(0.7f, CGPoint.ccp(0, 10 * Common.kYForIPhone)));
	    CCAction repeatAction = CCRepeatForever.action(moveSeq);
	    
	    int arrayCount = rivers1.size();
	    for (int i = 0; i < arrayCount; i ++) {
	        CCSprite river = rivers1.get(i);
	        river.runAction(repeatAction.copy());
	    }
	    
	    arrayCount = rivers2.size();
	    for (int i = 0; i < arrayCount; i ++) {
	        CCSprite river = rivers2.get(i);
	        river.runAction(repeatAction.copy());
	    }

	    arrayCount = rivers3.size();;
	    for (int i = 0; i < arrayCount; i ++) {
	        CCSprite river = rivers3.get(i);
	        river.runAction(repeatAction.copy());
	    }

	    arrayCount = rivers4.size();
	    for (int i = 0; i < arrayCount; i ++) {
	        CCSprite river = rivers4.get(i);
	        river.runAction(repeatAction.copy());
	    }
	...
	{    
	    ...
	    for (int i = 0; i < arrayCount; i ++) {
	        ...
	        if ( (body.getPosition().x + body.getContentSize().width * Common.kXForIPhone) < 0) {
	    ...
	    for (int i = 0; i < arrayCount; i ++) {
	        ...
	        if ( (body.getPosition().x + body.getContentSize().width * Common.kXForIPhone) < 0) {
	            body.setPosition(CGPoint.ccp(body.getPosition().x + Common.SCREEN_WIDTH * arrayCount, body.getPosition().y));
	...
	{
	    if (lands.size() > 0 && lands != null) {
	        
	        for (int i = 0; i < Common.STAGE_COUNT; i ++) {
	            CCSprite back = lands.get(i);
	            removeSprite(back);
	        }    
	        lands.removeAll(lands);
	    }
	    ...
	    arrayCount = rivers1.size();
	    if (arrayCount > 0 && rivers1 != null) {
	        for (int i = 0 ; i < arrayCount; i ++) {
	            CCSprite sprite = rivers1.get(i);
	            removeSprite(sprite);
	            sprite.removeFromParentAndCleanup(true);
	        }
	        rivers1.removeAll(rivers1);
	    }
	    arrayCount = rivers2.size();
	    if (arrayCount > 0 && rivers2 != null) {
	        for (int i = 0 ; i < arrayCount; i ++) {
	            CCSprite sprite = rivers2.get(i);
	            removeSprite(sprite);
	            sprite.removeFromParentAndCleanup(true);
	        }
	        rivers2.removeAll(rivers2);
	    }
	    arrayCount = rivers3.size();
	    if (arrayCount > 0 && rivers3 != null) {
	        for (int i = 0 ; i < arrayCount; i ++) {
	            CCSprite sprite = rivers3.get(i);
	            removeSprite(sprite);
	            sprite.removeFromParentAndCleanup(true);
	        }
	        rivers3.removeAll(rivers3);
	    }
	    arrayCount = rivers4.size();
	    if (arrayCount > 0 && rivers4 != null) {
	        for (int i = 0 ; i < arrayCount; i ++) {
	            CCSprite sprite = rivers4.get(i);
	            removeSprite(sprite);
	            sprite.removeFromParentAndCleanup(true);
	        }
	        rivers4.removeAll(rivers4);
	    }
