...
import com.siliconorchard.jumpingufo.Food;
import com.siliconorchard.jumpingufo.R;
import com.siliconorchard.jumpingufo.Common.GameState;
...
public class GameLayer extends CCLayer {
    ...
    {
        lblScoreLength = CCLabelAtlas.label("0123456789", "num4.png", 18, 24, '0');
        ...
        resumeMenuItem = CCMenuItemImage.item("resume.png", "resume.png", this, "onResume");
        ...
        retryMenuItem = CCMenuItemImage.item("retry.png", "retry.png", this, "onRetry");
        ...
        menuMenuItem = CCMenuItemImage.item("menu.png", "menu.png", this, "onMenu");
    ...
    private void initDot_Lines()
    {
        ...
        for (int i = 0; i < dotArrayCount; i ++) {
            ...
            dot_start.setScaleX(Common.kXForIPhone * 0.5f);
            dot_start.setScaleY(Common.kYForIPhone * 0.5f);
            ...
            dot_end.setScaleX(Common.kXForIPhone * 0.5f);
            dot_end.setScaleY(Common.kYForIPhone * 0.5f);
    ...
    private void initGame()
    {
        ...
        player.setPosition(10, START_POS_PLAYER_Y);
        player.realHeight = player.getPosition().y;
        player.tail.setScaleX(0.1f);
        player.tail.setVisible(false);
        player.state = Seed.SeedState_none;
        
        player.velocity = CGPoint.ccp(0, 0);
        player.state = Seed.SeedState_start;
        player.stateDirect = Seed.DirectState.DirectState_down;
    ...
    public boolean dotIntersectWithLine(Seed target, CGPoint line_start, CGPoint line_end)
    {
//        CGFloat dotVelY = target.velocity.y;
...
//            if (value_Y_inLine > dotPoint.y && dotVelY < 0) {
//                return TRUE;
//            }
            
            if (CGRect.containsPoint(targetRect, CGPoint.ccp(dotPoint.x, value_Y_inLine))) {
                if (player.state == Seed.SeedState_fail) {
                    player.face.stopAllActions();
                    player.face.runAction(CCRepeatForever.action((CCIntervalAction) Common.ani_seed_face_none.copy()));
                }
                return true;
            }
...
//        NSLog(@"alpha = %f", realAlpha * 180 / M_PI);
...
//        NSLog(@"abs Velocity = %f X= %f Y= %f", oldVelocityAbs, player.velocity.x, player.velocity.y);
        
        if (player.velocity.y > 400 * Common.kYForIPhone) {
            player.velocity = CGPoint.ccp(player.velocity.x, 400 * Common.kYForIPhone);
        }
        
        float nextTime = -player.velocity.y / Seed.GRAVITY * 2;
    ...
    public void restartGame()
    {
        ...
        float vel_Y = Common.SCREEN_WIDTH / 2 / DEFAULT_MOVE_VEL_X * Seed.GRAVITY / 2 * (-1);
        
        player.velocity = CGPoint.ccp(DEFAULT_MOVE_VEL_X, vel_Y);
//        player.startTime = [NSDate timeIntervalSinceReferenceDate];
    ...
    public void collisionScanWithFoods()
    {
        ...
        for (int i = 0; i < arrayCount; i ++) {
            ...
            if ( CGRect.intersects(player.rect(), food.rect()) ) {
                ...
                switch (food.type) {
                    case FoodType_bomb:
                        if (player.state == Seed.SeedState_protect) {
                            break;
                        }
...
//            completeGame();
    ...
    public void cameraMove()
    {
        ...
        moveLayer.setScaleX((float) (1 / ( 1 + cameraMoveDelta * 1.5f / Common.SCREEN_HEIGHT)));
        moveLayer.setScaleY((float) (1 / ( 1 + cameraMoveDelta * 1.5f / Common.SCREEN_HEIGHT)));

        tryAgain.setScale(moveLayer.getScaleX());
        keepOn.setScale(moveLayer.getScaleX());
        goolLuck.setScale(moveLayer.getScaleX());
    }
...
//        del.gameScore = player.flayLength / 3;
//        [del addOne];
//        [del submitScore];
        
        unschedule("startCountTime");
        gameState = Common.GameState.GameState_gameover;
//        player.endTime = [NSDate timeIntervalSinceReferenceDate];
        player.totalDistance += player.flayLength / 3;
        player.coinCount = player.sunCount / 10;
        Common.g_coinNum += player.coinCount;
        ...
        if(player.flayLength/3 > oldScore)
        {
        	SharedPref.putValue("topScore", (int)(player.flayLength/3));
...
//        [gameoverLayer.playerName setHidden:NO];
...
//        [player saveUserInfo];
	...
	public boolean ccTouchesBegan(MotionEvent event)
	{
	    ...
	    for (int i = 0; i < dotArrayCount; i ++) {
	        line = (BoardLine) board_lines.get(i);
	...
	public boolean ccTouchesEnded(MotionEvent event)
	{
	    ...
	    if (gameState == GameState.GameState_start) {
	        if (isStartNeed == true) {
//	            if (touchPoint.x < SCREEN_WIDTH / 3 * 2 && touchPoint.x > SCREEN_WIDTH / 2) {
	            if (touchPoint.x > Common.SCREEN_WIDTH / 2) {
